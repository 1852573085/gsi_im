...
public class Mp3EncodeThread extends Thread implements AudioRecord.OnRecordPositionUpdateListener {
    ...
    private static class StopHandler extends Handler {
        ...
        WeakReference<Mp3EncodeThread> encodeThread;

        public StopHandler(Mp3EncodeThread encodeThread) {
        ...
        public void handleMessage(Message msg) {
            if (msg.what == PROCESS_STOP) {
                Mp3EncodeThread threadRef = encodeThread.get();
    ...
    public Mp3EncodeThread(FileOutputStream os, int bufferSize) {
        this.os = os;
    ...
    private int processData() {
        if(mChangeBuffers != null && mChangeBuffers.size() > 0) {
            ...
            if (readSize > 0) {
                int encodedSize = Mp3Recorder.encode(buffer, buffer, readSize, mp3Buffer);
                if (encodedSize < 0) {
